...
FMG#07A                      DC    A(FMG#07)   * DDNameToPath
*
                             DS    0F
...
*        See if it's an MVS data set name
         IF (CLC,FNAMELENF04,LE,=A(54)) THEN
            ...
            L     R1,PARMFNAMEF04 * Point R1 to file name
            L     R3,FNAMELENF04
            IF (C,R3,GT,=A(46)) THEN
               L    R3,=A(46)
            ENDIF
            LR    R2,R1
            AR    R2,R3          * Search for max 45 characters
            SRST  R2,R1          * Scan for bracket
            BC    1,*-4          * Scan was incomplete, try again
*
            IF (4) THEN          * If ( found
...
* IFMG DDNameToPath
*
FMG#07   CEEENTRY AUTO=WORKDSAF07_SIZ,MAIN=NO,BASE=R10
*
         USING WORKDSAF07,R13    * Address DSA and extra stg for vars
*
         USING GLOBAL,R9         * Address global area DSECT
*
         L     R8,0(,R1)         * Parm 1 is object ptr
         USING FMG_obj,R8        * Address object DSECT
*
         MVC   PARMDDNAMEF07,4(R1) * Parm 2 is DD name
*
         L     R15,8(,R1)        * Parm 3 is Path
         ST    R15,PARMPATHF07   * Save in local var
*
         MVC   G_GETDSABD,G_GETDSABL
         L     R7,PARMDDNAMEF07
         GETDSAB DDNAME=0(,R7),DSABPTR=DSABPTRF07,MF=(E,G_GETDSABD)
*
         LA    R5,G_EPA
         ST    R5,G_SWEPAPTR
         USING ZB505,R5
