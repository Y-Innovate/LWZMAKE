...
*
            IPSS_Pop OBJECT=IPSS_ps_P04,WORK=WORKP04
*
            IF (CLC,G_ISTB_buildTargets,EQ,=A(0)) THEN
               MVC   G_ISTB_buildTargets,ti2.TFO_ISTB_token
            ELSE
               ISTB_AppendChar OBJECT=G_ISTB_buildTargets,WORK=WORKP04,X
               CHAR=C' '
               L     R2,ti2.TFO_ISTB_token
               L     R2,STB_lpBuf-STB_obj(,R2)
               ISTB_AppendZString OBJECT=G_ISTB_buildTargets,          X
               WORK=WORKP04,ZSTR=0(,R2)
            ENDIF
         ENDIF
*
         DROP  ti2,ti3
*
         IF (CLC,ti1.TFO_tokenType,EQ,=A(TOKEN_TYPE_DATASETSWITCH))
            IPSS_Push OBJECT=IPSS_ps_P04,WORK=WORKP04,                 X
               STATE_IN==A(PARSER_STATE_IN_DATASETPARM)
*
            IFMG_DDNameToDSName OBJECT=G_IFMG,WORK=WORKP04,            X
               DDNAME==CL8'LWZMTGT',DSNAME=datasetNameP04
*
            IF (CLC,datasetNameP04(3),EQ,=C'   ') THEN
               B     PRS#04_POP_DATASETPARM
            ELSEIF (CLC,datasetNameP04(3),EQ,=C'...') THEN
               MVI   datasetTypeP04,C'F'
            ELSE
               MVI   datasetTypeP04,C'D'
            ENDIF
*
            IF (CLI,datasetTypeP04,EQ,C'D') THEN DO
               MINSTANT GUID=G_IIND_GUID,WORK=WORKP04,                 X
               OBJPTR=IIND_TGTDS_P04
            ELSE
               MINSTANT GUID=G_IINF_GUID,WORK=WORKP04,                 X
               OBJPTR=IINF_TGTDS_P04
            ENDIF
*
            IIND_Open OBJECT=IIND_TGTDS_P04,WORK=WORKP04,              X
               DDNAME==CL8'LWZMTGT'
*
            MINSTANT GUID=G_IIFO_GUID,WORK=WORKP04,                    X
               OBJPTR=IIFO_TGTDS_P04
*
            MINSTANT GUID=G_ITOK_GUID,WORK=WORKP04,                    X
               OBJPTR=ITOK_TGTDS_P04
*
            ITOK_Init OBJECT=ITOK_TGTDS_P04,WORK=WORKP04,              X
               IIN=IIND_TGTDS_P04,IIFO=IIFO_TGTDS_P04
*
            IPSS_Push OBJECT=IPSS_ps_P04,WORK=WORKP04,                 X
               STATE_IN==A(PARSER_STATE_IN_TARGETPARM1)
*
            ITOK_GetNextToken OBJECT=ITOK_TGTDS_P04,WORK=WORKP04,      X
               IPSS=IPSS_ps_P04,ITFOPTR=ITFO_ti2_P04
*
            CLC   G_RETCODE,=A(0)
            BNE   PRS#04_RET
*
...
*              Get the next token
               ITOK_GetNextToken OBJECT=ITOK_TGTDS_P04,WORK=WORKP04,   X
               IPSS=IPSS_ps_P04,ITFOPTR=ITFO_ti3_P04
*
               CLC   G_RETCODE,=A(0)
               BNE   PRS#04_RET
*
...
*
               ...
               CLC,ti1.TFO_tokenType,NE,=A(TOKEN_TYPE_EOF))
                  IF (CLC,ti1.TFO_tokenType,EQ,=A(TOKEN_TYPE_COMMENT)),X
               OR,(CLC,ti1.TFO_tokenType,EQ,=A(TOKEN_TYPE_EOL)) THEN
*                    Get the next token
                     ITOK_GetNextToken OBJECT=ITOK_TGTDS_P04,          X
               WORK=WORKP04,IPSS=IPSS_ps_P04,ITFOPTR=ITFO_ti3_P04
*
                     L     R5,ITFO_ti3_P04
                  ELSE
               ...
               ENDDO
*
               IF (CLC,ti3.TFO_tokenType,EQ,=A(TOKEN_TYPE_EOF)) THEN
                  ASMLEAVE
               ENDIF
*
               IF (CLC,ti3.TFO_tokenType,EQ,=A(TOKEN_TYPE_VARIABLE))
...
*
            ...
            ENDDO
*
            IIND_Close OBJECT=IIND_TGTDS_P04,WORK=WORKP04
*
            IF (CLC,G_ISTB_buildTargets,EQ,=A(0)) THEN
               MVC   G_ISTB_buildTargets,ti2.TFO_ISTB_token
            ELSE
...
SCAN_EXPECTED_DATASETSWITCH  EQU   X'04'  * 22
*
...
*                  12345678901234567890123456789012
         ...
         DC    BL4'11000000000000000001110000000000' * PARAMETER
