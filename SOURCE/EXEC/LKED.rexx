...
g.LOAD.allocated = 0
g.LOAD.ddname.0 = 0
g.OPCLOAD.allocated = 0
g.OPCLOAD.ddname.0 = 0
...
IF g.error == 0 & g.opcload.0 /= 0 THEN DO
   _rc = BPXWDYN("ALLOC DSN('"g.opcload.1"') FI(OPCLOAD) SHR REUSE")

   IF _rc == 0 THEN DO
      g.OPCLOAD.allocated = 1
      g.OPCLOAD.ddname.0 = 1
      g.OPCLOAD.ddname.1 = "OPCLOAD"
   END
   ELSE DO
      g.error = 8
      IF _rc > 0 THEN _rc = D2X(_rc)
      CALL log 'Dynamic allocation of 'g.opcload.1' failed with '_rc
   END

   DO i = 2 TO g.opcload.0 WHILE g.error == 0
      _rc = BPXWDYN("ALLOC DSN('"g.opcload.i"') SHR RTDDN(_ddn)")

      IF _rc == 0 THEN DO
         _nextDD = g.OPCLOAD.ddname.0 + 1
         g.OPCLOAD.ddname.0 = _nextDD
         g.OPCLOAD.ddname._nextDD = _ddn

         _rc = BPXWDYN("CONCAT DDLIST(OPCLOAD,"_ddn")")
      ...
      ELSE DO
         g.error = 8
         IF _rc > 0 THEN _rc = D2X(_rc)
         CALL log 'Dynamic allocation of 'g.opcload.i' failed with '_rc
      END
   END
END
...
IF g.OPCLOAD.allocated == 1 THEN DO
   DO i = 1 TO g.OPCLOAD.ddname.0
      _rc = BPXWDYN("FREE FI("g.OPCLOAD.ddname.i")")

      IF _rc /= 0 THEN DO
         g.error = 8
         IF _rc > 0 THEN _rc = D2X(_rc)
         CALL log 'Free of file 'g.OPCLOAD.ddname.i' failed with '_rc
      END
   END
...
DO WHILE g.error == 0
   ...
   WHEN _parmName == 'LOAD' THEN DO
      ...
      END
   END
   WHEN _parmName == 'OPCLOAD' THEN DO
      g.parser.scanState = g.parser.SCAN_STATE_IN_DSNAME1
      CALL lexerGetToken
      IF g.error /= 0 | g.scanner.currChar == 'EOF' THEN LEAVE
      DO WHILE g.lexer.currToken /= ')'
         ...
         _dsname = parseDsname()
         IF g.error /= 0 | g.scanner.currChar == 'EOF' THEN LEAVE
         _nextLoad = g.opcload.0 + 1
         g.opcload.0 = _nextLoad
         g.opcload._nextLoad = _dsname
         g.parser.scanState = g.parser.SCAN_STATE_IN_PARM3
         CALL lexerGetToken
         IF g.error /= 0 | g.scanner.currChar == 'EOF' THEN LEAVE
      END
...
IF g.error == 0 THEN DO
   ...
   DO i = 1 TO g.load.0
      SAY 'LOAD:         'g.load.i
   END
   DO i = 1 TO g.opcload.0
      SAY 'OPCLOAD:      'g.opcload.i
   END
