...
IF _rc == 0 THEN DO
   SAY "BIND output copied to DD "_ddn
   ...
   IF _rc /= 0 THEN DO
      g.error = 8
      CALL log "EXECIO DISKW for BIND output failed "_rc
   END
   ...
   IF _rc /= 0 THEN DO
      g.error = 8
      CALL log "FREE for copy BIND output failed "_rc
   END
END
ELSE DO
   g.error = 8
   CALL log "ALLOC for copy BIND output failed "_rc
END
...
g.parser.EXPECTED_ASTERISK = 256
